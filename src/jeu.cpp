...
bool Jeu::execution(Couleur couleur, string str_orig, string str_dest, string coup_prec, bool *prise_ou_pion) {
    ...
    if (str_orig == str_dest) {
        ...
        if (echiquier.get_piece(dest) == nullptr) { // Si la case de destination est vide
            ...
            cerr << endl << ANSI_COLOR_RED << "La case de destination est occupée par une de vos pièces, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
...
bool Jeu::petit_roque(Couleur couleur) {
    if (couleur == Blanc) {
        if (echiquier.echec(couleur, false)) {
            cerr << endl << ANSI_COLOR_RED << "Impossible de réaliser le petit roque, votre roi est en échec, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
            ...
            cerr << endl << ANSI_COLOR_RED << "Impossible de réaliser le petit roque, le roi ou la tour a été déplacé, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else if (echiquier.get_piece(0, 5) || echiquier.get_piece(0, 6)) {
            cerr << endl << ANSI_COLOR_RED << "Impossible de réaliser le petit roque, les cases sont occupées, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else {
            if (echiquier.deplace(Square(0, 4), Square(0, 5)) == false) {
                cerr << ANSI_COLOR_RED << "Impossible de réaliser le petit roque, votre roi est menacé à un moment, veuillez réessayer !" << ANSI_COLOR_RESET << endl;
                return false;
            };
...
bool Jeu::grand_roque(Couleur couleur) {
    if (couleur == Blanc) {
        if (echiquier.echec(couleur, false)) {
            cerr << endl << ANSI_COLOR_RED << "Impossible de réaliser le grand roque, votre roi est en échec, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
            ...
            cerr << endl << ANSI_COLOR_RED << "Impossible de réaliser le grand roque, le roi ou la tour a été déplacé, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else if (echiquier.get_piece(0, 1) || echiquier.get_piece(0, 2) || echiquier.get_piece(0, 3)) {
            cerr << endl << ANSI_COLOR_RED << "Impossible de réaliser le grand roque, les cases sont occupées, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else {
            if (echiquier.deplace(Square(0, 4), Square(0, 3)) == false) {
                cerr << ANSI_COLOR_RED << "Impossible de réaliser le grand roque, votre roi est menacé à un moment, veuillez réessayer !" << ANSI_COLOR_RESET << endl;
                return false;
            };
