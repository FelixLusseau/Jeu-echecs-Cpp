...
bool Jeu::test_mat(Couleur couleur) { return echiquier.mat_ou_pat(couleur, false, 0); }

bool Jeu::test_pat(Couleur couleur, int compteur) { return echiquier.mat_ou_pat(couleur, true, compteur); }

bool Jeu::jeu(Couleur couleur, string str_orig, string str_dest, string coup_prec, bool *prise_ou_pion) {
    Square orig(str_orig), dest(str_dest);
    if (str_orig == str_dest) {
        cout << endl << ANSI_COLOR_RED << "Mouvement ne déplaçant pas la pièce (si elle existe), veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
        return false;
    } else if (dest.get_ligne() < 0 || dest.get_ligne() >= NBCOL || dest.get_colonne() < 0 || dest.get_colonne() >= NBCOL) {
        cout << endl << ANSI_COLOR_RED << "Mouvement en dehors de la grille, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
        return false;
    } else if (echiquier.get_piece(orig) == nullptr) {
        cout << endl << ANSI_COLOR_RED << "La case d'origine est vide ! " << ANSI_COLOR_RESET << endl;
        return false;
    } else if (echiquier.get_piece(orig)->get_couleur() != couleur) {
        cout << endl << ANSI_COLOR_RED << "Cette pièce ne vous appartient pas, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
        return false;
        ...
        if (echiquier.get_piece(dest) == nullptr) { // Si la case de destination est vide
            ...
            if (coup_prec.length() > 0 && echiquier.get_piece(orig)->get_type() == "Pion" && (orig.get_colonne() - dest.get_colonne() == 1 || orig.get_colonne() - dest.get_colonne() == -1)) {
                ...
                if (dest.get_ligne() == 5 && cp_orig.get_colonne() == dest.get_colonne() && cp_orig.get_ligne() == 6 && cp_dest.get_ligne() == 4) {
                    ...
                    *prise_ou_pion = true;
                } else if (dest.get_ligne() == 2 && cp_orig.get_colonne() == dest.get_colonne() && cp_orig.get_ligne() == 1 && cp_dest.get_ligne() == 3) {
                    echiquier.prise_en_passant(cp_dest.get_ligne(), cp_dest.get_colonne());
                    *prise_ou_pion = true;
                } else {
                    cout << endl << ANSI_COLOR_RED << "Déplacement en diagonale invalide pour un pion, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
                    return false;
                }
            }
            ...
            if (echiquier.get_piece(orig)->get_type() == "Pion" && (orig.get_colonne() - dest.get_colonne() == 1 || orig.get_colonne() - dest.get_colonne() == -1)) {
                cout << endl << ANSI_COLOR_RED << "Déplacement en diagonale invalide pour un pion, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
                return false;
            }
            cout << endl << ANSI_COLOR_RED << "La case de destination est occupée par une de vos pièces, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
            ...
            if (echiquier.get_piece(orig)->get_type() == "Pion" && orig.get_colonne() - dest.get_colonne() == 0) {
                cout << endl << ANSI_COLOR_RED << "Prise en avançant invalide pour un pion, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
                return false;
            }
        ...
        cout << endl << ANSI_COLOR_RED << "Mouvement invalide, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
        return false;
    }
}
...
bool Jeu::petit_roque(Couleur couleur) {
    if (couleur == Blanc) {
        if (echiquier.echec(couleur, false)) {
            cout << endl << ANSI_COLOR_RED << "Impossible de réaliser le petit roque, votre roi est en échec, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else if ((echiquier.get_piece(0, 4) && echiquier.get_piece(0, 4)->get_a_bouge()) || !echiquier.get_piece(0, 4) || (echiquier.get_piece(0, 7) && echiquier.get_piece(0, 7)->get_a_bouge()) ||
                   !echiquier.get_piece(0, 7)) {
            cout << endl << ANSI_COLOR_RED << "Impossible de réaliser le petit roque, le roi ou la tour a été déplacé, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else if (echiquier.get_piece(0, 5) || echiquier.get_piece(0, 6)) {
            cout << endl << ANSI_COLOR_RED << "Impossible de réaliser le petit roque, les cases sont occupées, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else {
            if (echiquier.deplace(Square(0, 4), Square(0, 5)) == false) {
                cout << ANSI_COLOR_RED << "Impossible de réaliser le petit roque, votre roi est menacé à un moment, veuillez réessayer !" << ANSI_COLOR_RESET << endl;
                return false;
            };
        ...
        if (echiquier.echec(couleur, false)) {
            cout << endl << ANSI_COLOR_RED << "Impossible de réaliser le petit roque, votre roi est en échec, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else if ((echiquier.get_piece(7, 4) && echiquier.get_piece(7, 4)->get_a_bouge()) || !echiquier.get_piece(7, 4) || (echiquier.get_piece(7, 7) && echiquier.get_piece(7, 7)->get_a_bouge()) ||
...
bool Jeu::grand_roque(Couleur couleur) {
    if (couleur == Blanc) {
        if (echiquier.echec(couleur, false)) {
            cout << endl << ANSI_COLOR_RED << "Impossible de réaliser le grand roque, votre roi est en échec, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else if ((echiquier.get_piece(0, 4) && echiquier.get_piece(0, 4)->get_a_bouge()) || !echiquier.get_piece(0, 4) || (echiquier.get_piece(0, 0) && echiquier.get_piece(0, 0)->get_a_bouge()) ||
                   !echiquier.get_piece(0, 0)) {
            cout << endl << ANSI_COLOR_RED << "Impossible de réaliser le grand roque, le roi ou la tour a été déplacé, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else if (echiquier.get_piece(0, 1) || echiquier.get_piece(0, 2) || echiquier.get_piece(0, 3)) {
            cout << endl << ANSI_COLOR_RED << "Impossible de réaliser le grand roque, les cases sont occupées, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else {
            if (echiquier.deplace(Square(0, 4), Square(0, 3)) == false) {
                cout << ANSI_COLOR_RED << "Impossible de réaliser le grand roque, votre roi est menacé à un moment, veuillez réessayer !" << ANSI_COLOR_RESET << endl;
                return false;
            };
        ...
        if (echiquier.echec(couleur, false)) {
            cout << endl << ANSI_COLOR_RED << "Impossible de réaliser le grand roque, votre roi est en échec, veuillez réessayer ! " << ANSI_COLOR_RESET << endl;
            return false;
        } else if ((echiquier.get_piece(7, 4) && echiquier.get_piece(7, 4)->get_a_bouge()) || !echiquier.get_piece(7, 4) || (echiquier.get_piece(7, 0) && echiquier.get_piece(7, 0)->get_a_bouge()) ||
